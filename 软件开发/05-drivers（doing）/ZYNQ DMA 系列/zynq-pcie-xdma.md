...
如下图所示，把流模式的工作场景理解成一根管子，往管子里放刚好内径大小的球。A端作为管子的输入端，B端最为管子的输出端。比如时刻1往管子A端放进去一颗红色球，接着时刻2往管子A端放进去一颗绿色球，时刻3往A端放进去蓝色球。那么作为接收端，只能从管子B端接收球，且接收端无法先拿到绿色球，接收端必须先拿到红色球，才能拿到绿色球，接着拿到蓝色球。这样来解释无地址概念。

![[Pasted image 20260626160331.png]]


AXI-MM 模式与 Burst突发传输大小，假设Burst=16，假如此刻来了18个小球，burst=16表示单次 AXI 传输事务最大为16，而本例子中，18个会拆分成2个传输事务来处理。

![[Pasted image 20260626172402.png]]
