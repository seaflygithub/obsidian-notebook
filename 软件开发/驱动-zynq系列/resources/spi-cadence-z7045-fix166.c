...
#if 0 // 强制指定分频系数为4
	{
		//printk("%s:%s():z7045:Set the SPI device pre-scalar to divide by 4\r\n",__FILE__,__func__);
...
static int cdns_spi_probe(struct platform_device *pdev)
{
	...
	printk("%s:%s(): z7045: get pclk is %ld\r\n", __FILE__, __func__, clk_get_rate(xspi->pclk));
	...
	printk("%s:%s(): z7045: get ref_clk is %ld\r\n", __FILE__, __func__, clk_get_rate(xspi->ref_clk));
	...
	printk("%s:%s(): z7045: enabled APB clock (pclk)\r\n", __FILE__, __func__);




	// z7045: force set ref_clk is 133333333
	// z7045: force set ref_clk is 166666667
	clk_set_rate(xspi->ref_clk, 166666667);
	printk("%s:%s(): z7045: Force set spi device ref clock is %ld\r\n", 
			__FILE__, __func__, 166666667);
	printk("%s:%s(): z7045: The final spi device ref clock is %ld\r\n", 
			__FILE__, __func__,clk_get_rate(xspi->ref_clk));


	ret = clk_prepare_enable(xspi->ref_clk);
	if (ret) {
		dev_err(&pdev->dev, "Unable to enable device clock (ref_clk).\n");
		goto clk_dis_apb;
	}
	printk("%s:%s(): z7045: enabled spi device ref clock (ref_clk)\r\n", __FILE__, __func__);
	...
	printk("%s:%s(): z7045: default max_speed_hz is %ld\r\n", __FILE__, __func__, master->max_speed_hz);
	printk("%s:%s(): z7045: default speed_hz is %ld\r\n", __FILE__, __func__, xspi->speed_hz);
